...
const WaButton: React.FC<WaProps> = ({ pesan }) => {
    const formatMessage = (pesan: { id: any; name?: string;}) => {
        return `Pesanan: 
    Id produk: ${pesan.id}
    Nama produk: ${pesan.name}
    Total pesanan: `
    }

    const message = formatMessage(pesan);
    ...
    return (
        <Link
            ...
            target="_blank"
            className="mt-2 bg-green-500 text-white flex items-center justify-center gap-4 rounded-box shadow hover:bg-green-400 p-2"
